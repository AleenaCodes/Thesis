...
A sectioning root is an HTML element that can have its own outline, but the sections and headings inside it do not contribute to the outline of its ancestor. The sectioning root \texttt{<figure>} is the key one that I used in my development, to create an element that was able to have many children (i.e. axis, datapoints etc.) and not create a lot of excess outline in the DOM for the parent section.
...
% Had to add aria-label to the svg list item as Voiceover wouldn't pick up on it 
...
% - Had to use CreateElementNS and SetAttributeNS as SVGs are a bit fiddly (find the technical term - different namespace?)
