...
Inlining SVG provides more predictable results and better control over properties than adding in SVG files with a \textt{<img>} or \textt{<use>} tag. This is because the SVG source is then directly available in the DOM, which is exposed by the accessibility API used by assistive technologies.
...
One of the new features in this specication is the abilities to add \textt{tabindex} to SVG elements (such as shapes or text). This is useful, as previously the only way to make elements in an SVG that were focusable by keyboard, was by including a HTML element that supported this, such as the \textt{<a>} tag.
...
As discussed before, I aimed to create as much of the accessibility tree as possible from using inbuilt HTML properties (e.g. using \textt{<h1>}) for headers as opposed to adding an ARIA role to achieve this.
...
% Look at git commits on 4/4 and 5/4 for reference of changes made
