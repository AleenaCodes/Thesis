...
%TODO - add some pics of the evaluating tools
...
As screen-reader users are a small group compared to all web users, I intended to try to run only a small round of user testing - around 5 or so users. Overall I had 7 users in the study.

The study consisted of 3 main parts

\begin{itemize}
    \item Background questions
    \begin{itemize}
        \item Before showing any content to the user I asked some background questions about their usage of screen-readers
        \item The questions focused on their current perception around if they found images and charts on web pages accessible
        \item I also asked some questions around how long they'd used a screen-reader, and which operating system, browser and screen-reader they used
    \end{itemize}
...
% TODO - picture of demo

I tried to keep the study short for the user (less than 30 minutes) and so the demo was only one of each types of graph, and both surveys combined were 11 questions, mainly multiple-choice.
...
% screen-reader pie
% how long using screen-reader pie
% proficient screen-reader pie

% images accessible pie
% tabular data accessible pie
% data viz accessible pie
% tabular data accessible pie

The post-demo questions had results as follows

% understand subject pie
% undertand data pie
% info given pie

% Understand how to navigate pie
% notes on answers for changing nav key
...
\begin{itemize}
    \item Most users had \texit{Windows} as their operating system, and \textit{Chrome} as their browser
    ...
    \item The only user not using \textit{JAWS} or \textit{NVDA} was using \textit{Voiceover} - a popular choice for \textit{MacOS} and \textit{iOS} users due to being native to the operating system
    \item The users were mainly long-term screen-reader users (10+ years), with only one new (0-1 years) screen-reader user in the pool
    \item Following on from this, the users mainly rated themselves as "Expert" in their proficiency using a screen-reader
\end{itemize}
...
\begin{itemize}
    ...
    \item A small percentage (25\% for images and tables, 12\% for data visualisations) said that they "Understand Most" for any of these types of visuals
    \item More than 60\% of users said that the "Struggle to Understand" data visualisations on web pages, so it seems that that many developers do not provide adequate descriptions for these, possible because it can be complex to sum up large amounts of visual data succinctly
\end{itemize}

% TODO -evaluate post-demo questions
